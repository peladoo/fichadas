-- ========================================
-- BOOTSTRAP: proyecto Supabase NUEVO (vacío)
-- ========================================
-- Usá ESTE archivo si no tenés tablas todavía.
-- No corras saas_multitenant.sql en un proyecto vacío.
--
-- 1. Creá un proyecto en https://supabase.com (Free, región São Paulo).
...
-- 5. Copiá URL, anon key y service_role a .env.local
-- 6. pnpm dev → http://localhost:3000  y  /platform
--
-- Si ya corriste saas_multitenant.sql y falló a mitad, este archivo
-- es idempotente (IF NOT EXISTS). Podés correrlo igual.
...
-- ----------------------------------------
-- 2. Tablas de negocio (completas, con municipio_id)
-- ----------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_fichadas_dependencia ON fichadas (dependencia_id);
CREATE INDEX IF NOT EXISTS idx_fichadas_tipo ON fichadas (tipo);
...
-- Dependencias de prueba para San Benito
INSERT INTO dependencias (
  municipio_id, nombre, codigo, direccion, latitud, longitud, radio_metros
)
SELECT m.id, v.nombre, v.codigo, v.direccion, v.latitud, v.longitud, v.radio
FROM municipios m
CROSS JOIN (VALUES
  ('CIC', 'CIC-001', 'Calle Garay y Nogoya', -31.771765084997554, -60.42306082350464, 100),
  ('NIDO', 'NIDO-001', 'Calle Misiones y Buenos Aires', -31.777360074698333, -60.43369446481205, 100)
) AS v(nombre, codigo, direccion, latitud, longitud, radio)
WHERE m.slug = 'san-benito'
ON CONFLICT (municipio_id, codigo) DO UPDATE SET
  latitud = EXCLUDED.latitud,
  longitud = EXCLUDED.longitud,
  radio_metros = EXCLUDED.radio_metros,
  direccion = EXCLUDED.direccion;

-- Usuarios Auth existentes → owner de San Benito
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'No existe un usuario Auth con email %. Crealo antes en Authentication → Users.', user_email;
  END IF;
END;
$$;
...
CREATE POLICY "anon lee dependencias activas" ON dependencias
  FOR SELECT TO anon
  USING (municipio_id IN (SELECT id FROM municipios WHERE activo));
...
VALUES ('fotos-fichadas', 'fotos-fichadas', false)
